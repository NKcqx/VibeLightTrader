...
> **致 Claude（接收的你）**: 这条 packet 是 equity-monitor 自动生成的，
...
你之前跟用户讨论过 equity-monitor 的设计，下面这些上下文你**必须**召回：
...
equity-monitor 会自动消费 submitted 目录里的决策，过硬约束 + 写 Trade /
...
def render_packet_md(p: DecisionPacket, *, repo_root: Path | None = None) -> str:
    ...
    (which is what `equity-monitor` runs in anyway).
    """
...
def default_memory_hints(repo_root: Path | None = None, code: str = "") -> list[str]:
    ...
    them via the Shell tool. Tuned for the equity-monitor repo layout.
    """
    repo = str(repo_root) if repo_root else "/Users/bytedance/Documents/Code/equity-monitor"
    ...
    db = f"{repo}/data/equity_monitor.db"
    ...
    hints += [
        ...
        f"rg -n 'def explain_signal' {repo}/src/equity_monitor/reports/render.py",
    ]
    return hints
