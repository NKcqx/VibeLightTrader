...
from vibe_trader.config import load_settings, load_watchlist
from vibe_trader.events.grammar import ALLOWED_CHART_FREQS
from vibe_trader.data.backfill import backfill_all
    ...
    if "cfg" not in ctx.obj:
        ...
        if not Path(path).exists():
            raise click.UsageError(
                ...
                f"Either `cd` into the vibe-trader repo (so 'config/settings.yaml' "
                f"resolves), or pass an absolute path: "
                f"`vibe-trader --settings /abs/path/to/settings.yaml ...`."
            )
...
def listen(
    ...
    Pair with `vibe-trader run` in another tmux pane.
    """
...
def chart(
    ...
    Examples:
      vibe-trader chart US.AAPL
      vibe-trader chart US.NVDA --freq D
      vibe-trader chart US.AAPL --freq 15m --push
    """
    from pathlib import Path
...
def analyze(
    ...
    Examples:
      ...
      vibe-trader analyze
      vibe-trader analyze --code US.NVDA --code US.MSFT
      vibe-trader analyze --code US.NVDA --budget 30000 --drawdown 15
      vibe-trader analyze --execute       # main use case: act on the LLM
    """
    import json as _json
    from datetime import datetime as _dt

    from vibe_trader.analyze import analyze_symbols
...
def _execute_analysis_results(
    ...
    paper trade. Mirrors the logic of `vibe-trader trade confirm`,
...
def decide_list(ctx: click.Context, state: str) -> None:
    ...
    from vibe_trader.decisions.store import PacketState
...
def decide_submit(
    ...
    try:
        with session_scope(factory) as s:
            ...
            if sym is None:
                raise click.ClickException(
                    f"symbol {p.code!r} not in DB; run `vibe-trader watchlist sync` first"
                )
            sig = SignalRow(
                symbol_id=sym.id,
